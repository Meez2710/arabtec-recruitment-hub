...
const compressed=await fetch(B+'/vendor/babel.min.js',{headers:{'Accept-Encoding':'gzip'}});
c('large browser asset is gzip-compressed on direct HTTP',compressed.headers.get('content-encoding')==='gzip');
const compiler=await compressed.text();
c('compressed asset decompresses to the original JavaScript',compiler.length>1000000 && compiler.includes('Babel'));
